...
var parseLength = function(value, normalize){
    ...
    if (!match) return value // cannot be parsed. probably "auto"
...
var setter = function(key){
    return setters[key] || (setters[key] = (function(){
        ...
        return function(value){
            this.style[alias] = parser(value, false, this)
        }
...
var BrowserAnimation = prime({
    ...
    prepare: function(to){
        ...
        }/*(es5)?*/else if (p === parseTransform2d) (function(){ // IE9/Opera
...
nodes.implement({
    ...
    compute: function(property){
        var p    = parsers[property = camelize(property)],
            node = this[0]

        /*(es5)?*/

        if (p === parseTransform2d) return compute(node)(transformName)

        var value  = getter(property).call(node)

        // unit conversion to `px`

        if (p && (p === parseLength || p === parseBorder || p === parseShort4)) value = value.replace(rgLength, function(match, value, unit){
