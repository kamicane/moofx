...
var round = function(number, x){
    return +Number.prototype.toPrecision.call(+number, x || 3)
}
...
var parseLength = function(value, normalize, node){
    ...
    return (node && unit !== "px") ? round(pixelRatio(node, unit) * value) + "px" : round(value) + unit
}
...
switch (aliases.transition){
    case "MozTransition"    : transitionEndName = "transitionend"       ; break
    case "WebkitTransition" : transitionEndName = "webkitTransitionEnd" ; break
    case "OTransition"      : transitionEndName = "OTransitionEnd"      ; break
    case "msTransition"     : transitionEndName = "msTransitionEnd"     ; break
}
...
var CSSAnimation = prime({
    ...
    defer: function(time){
        this.running = true
        this.timer = setTimeout(this.bcomplete, this.duration)
